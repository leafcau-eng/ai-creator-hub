#!/usr/bin/env python3
# patch-ai-providers.sh — replace lib/ai-providers.ts dengan fallback chain
...
NEW_CONTENT = r"""// lib/ai-providers.ts
...
checks = ["callGemini", "callGroq", "callDeepSeek", "callOpenRouter", "All providers failed"]
ok = all(c in verify for c in checks)
if ok:
    print("SUKSES: ai-providers.ts berhasil diganti dengan fallback chain.")
else:
